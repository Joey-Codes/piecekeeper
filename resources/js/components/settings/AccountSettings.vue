        ...
        <div class="space-y-4 sm:space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2.5 sm:gap-3">
                    ...
                    <p
                        v-else
                        class="text-sm sm:text-base text-stone-800"
                    >
                        {{ accountEmail }}
                    </p>
                </div>
                <!-- Google badge or email verification status -->
                <div class="flex items-center gap-2 mt-2">
                    <span
                        v-if="hasGoogle"
                        class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm font-semibold rounded-full bg-blue-50 text-blue-700 border border-blue-200"
                    >
                        <svg
                            class="w-3.5 h-3.5"
                            viewBox="0 0 24 24"
            ...
            <div
                v-if="hasPassword"
                class="border-t border-stone-200/60 pt-4 sm:pt-5"
            >
...
const hasGoogle = computed(() => !!auth.user?.has_google)
const hasPassword = computed(() => !!auth.user?.has_password)

const accountDirty = computed(() => {
    const nameDirty = accountName.value !== (auth.user?.name || '')
    const emailDirty = !hasGoogle.value && accountEmail.value !== (auth.user?.email || '')
    return nameDirty || emailDirty
})
...
async function saveAccount() {
    ...
    try {
        ...
        if (!hasGoogle.value && accountEmail.value !== (auth.user?.email || '')) data.email = accountEmail.value
