  ...
  <div class="min-h-screen bg-white">
    <div class="relative max-w-4xl mx-auto px-6 py-12">
      <header class="mb-10">
        ...
        <p class="font-semibold text-stone-500 mt-1">A snapshot of your piano journey</p>
      </header>
        ...
        >
          <div class="flex items-center gap-3 mb-3">
            <div
              ...
              :class="stat.bgClass"
            >
              <span class="text-white text-sm" v-html="stat.icon"></span>
            </div>
            <p class="text-md font-semibold text-stone-800 uppercase tracking-wide">{{ stat.label }}</p>
          </div>
          <p class="text-3xl font-bold text-stone-800">
            {{ stat.value }}<span v-if="stat.unit" class="text-sm font-semibold text-stone-500 ml-1">{{ stat.unit }}</span>
          </p>
          <p class="text-md font-semibold text-stone-500 mt-1">{{ stat.detail }}</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3 mt-3 text-sm font-semibold text-stone-400">
        ...
        <div class="flex gap-1">
          <div class="w-3 h-3 rounded bg-stone-100"></div>
          <div class="w-3 h-3 rounded bg-amber-200"></div>
          <div class="w-3 h-3 rounded bg-amber-400"></div>
          <div class="w-3 h-3 rounded bg-amber-600"></div>
        </div>
        <span>More</span>
      </div>
    </div>
